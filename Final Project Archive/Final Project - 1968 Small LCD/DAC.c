...
output = 0;
  ...
  GPIO_PORTE_AFSEL_R |= 0x0F;           // enable alt funct on PA2,3,5
  GPIO_PORTE_DEN_R |= 0x0F;             // enable digital I/O on PA2,3,5
	delay = SYSCTL_RCGC2_R;
	SSI0_CR1_R &= ~SSI_CR1_SSE;
  SSI0_CR1_R &= ~SSI_CR1_SSE;           // disable SSI
  SSI0_CR1_R &= ~SSI_CR1_MS;            // master mode (default setting)
                                        // clock divider for 3 MHz SSIClk
  SSI0_CPSR_R = (SSI1_CPSR_R&~SSI_CPSR_CPSDVSR_M)+2;
  SSI0_CR0_R &= ~(SSI_CR0_SCR_M |       // SCR = 0 (3 Mbps data rate) (default setting)
                  SSI_CR0_SPO);         // SPO = 0 (default setting)
	SSI0_CR1_R |= (SSI_CR0_SPH);         // SPH = 1
	//SSI1_CPSR_R = ;
	//SSI1_CR0_R &= 0xFFCF;
                                        // FRF = Freescale format (default setting)
  SSI0_CR0_R = (SSI1_CR0_R&~SSI_CR0_FRF_M)+SSI_CR0_FRF_MOTO;
                                        // DSS = 16-bit data
  SSI0_CR0_R = (SSI1_CR0_R&~SSI_CR0_DSS_M)+SSI_CR0_DSS_16;
