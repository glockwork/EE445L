...
RITWriteCommand(const unsigned char *pucBuffer, unsigned long ulCount)
{
    ...
    while(ulCount != 0)
    {
        ...
        //
        SSIDataPut(SSI1_BASE, *pucBuffer++);
...
RIT128x96x4Enable(unsigned long ulFrequency)
{
    ...
    //
    // Configure the SSI1 port for master mode.
    //
    ...
    GPIOPadConfigSet(GPIO_PORTE_BASE, GPIO_PIN_1, GPIO_STRENGTH_8MA,
...
RIT128x96x4Init(unsigned long ulFrequency)
{
    ...
    //
    // Enable the SSI1 and GPIO port blocks as they are needed by this driver.
    //
    ...
    //
    // Configure the SSI1CLK and SSIOTX pins for SSI operation.
    //
    ...
    GPIOPadConfigSet(GPIO_PORTE_BASE, GPIO_PIN_0 | GPIO_PIN_1 | GPIO_PIN_3,
                     GPIO_STRENGTH_8MA, GPIO_PIN_TYPE_STD_WPU);
    ...
    //
    // Configure and enable the SSI1 port for master mode.
    //
